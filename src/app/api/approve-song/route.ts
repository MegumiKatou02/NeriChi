    ...
    try {
      if (originalSongId) {
        ...
        if (originalSongSnapshot.exists()) {
          ...
          const { createdAt, updatedAt, originalSongId, id, ...cleanedData } = pendingSongData

          console.log(createdAt, updatedAt, originalSongId, id)

          const versions = cleanedData.versions

          const versionContributors = Object.values(
            versions as Record<string, { contributors?: string[] }>,
